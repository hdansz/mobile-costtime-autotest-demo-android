...
def runAllImageSimilaryFun ( para1, para2 ):
    ...
    print('[均值]哈希(值越小，越相似0-64，如0)相似度aHash：', n1)

    hash1 = dHash(img1)
    hash2 = dHash(img2)
    n2 = cmpHash(hash1, hash2)
    print('[差值]哈希(值越小，越相似0-64，如0)相似度dHash：', n2)

    hash1 = pHash(img1)
    hash2 = pHash(img2)
    n3 = cmpHash(hash1, hash2)
    print('[感知]哈希(值越小，越相似0-64，如0)相似度pHash：', n3)

    n4 = classify_hist_with_split(img1, img2)
    print('三[直方图](值越大越相似0-1，如1)相似度：', n4)

    n5 = calculate(img1, img2)
    print("单通道的[直方图](值越大越相似0-1，如1)", n5)
    # print("%d %d %d %.2f %.2f " % (n1, n2, n3, round(n4[0], 2), n5[0]))
    # print("%.2f %.2f %.2f %.2f %.2f " % (
    # 1 - float(n1 / 64), 1 - float(n2 / 64), 1 - float(n3 / 64), round(n4[0], 2), n5[0]))
...
if __name__ == "__main__":
    ...
    #p2 =path + "/temp_screenshot/" + "test.jpg"(P1=P2)
    print('这是相似度不高的P1/P2图片（差值哈希算法：22分，相对不那么相似，比较准确）')
